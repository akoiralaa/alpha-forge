"""Tier 1: Rule-based signals. Stateless functions of MarketStateVector.

All return a score in [-1, 1]:
...
def signal_mean_reversion(msv, threshold: float = 2.0) -> float:
    """Mean-reversion using zscore_20 (primary) and zscore_100 (confirmation).

    Negative Z-score → positive signal (buy the dip).
    Positive Z-score → negative signal (fade the rally).
    """
...
def signal_momentum(msv, threshold: float = 0.5) -> float:
    """Momentum: composite of 60s and 300s returns."""
...
def signal_volume_anomaly(msv, vol_threshold: float = 2.0) -> float:
    """Volume anomaly: high volume + direction = signal."""
